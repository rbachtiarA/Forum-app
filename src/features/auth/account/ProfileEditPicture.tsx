        ...
        <CardContent className="flex flex-col justify-center items-center gap-2 h-full">
            <div className="relative group">
                <div className="relative w-[200px] h-[200px] overflow-hidden cursor-pointer rounded-full">
                    <div onClick={handleClick} className="absolute w-full h-full rounded-full flex flex-col justify-center items-center bg-black opacity-0 hover:opacity-65 transition-all duration-300 ease-in-out gap-2">
                        <CameraIcon className="text-white"size={30} />
                        <p className="text-white font-bold">Change profile picture</p>
                    </div>
                    <img 
                    src={!selectedPreviewImage? '/static/kriibo-default-avatar.png' : selectedPreviewImage} 
            ...
            <input ref={profilePicture} type='file' className="hidden" accept="image/*" onChange={handleImageChange} />
        </CardContent>
        <CardFooter className="flex gap-2 justify-center p-0">
            <Button disabled={!isDirty} onClick={handleApply}>Apply</Button>
            <Button onClick={handleReset} className={`${isDirty ? '': 'hidden'}`} disabled={!isDirty} variant={'destructive'}>Reset</Button>
            <Button onClick={handleDelete} className="font-bold" disabled={!data?.picture} variant={'destructive'}>Remove picture</Button>
        </CardFooter>
    </Card>
  )
}
